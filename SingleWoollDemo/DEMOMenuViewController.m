...
- (void)viewDidLoad
{
    ...
    self.tableView.tableHeaderView = ({
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 0, 264.0f)];
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 200, 0, 24)];
        label.text = @"BRXT";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        NSArray *titles = @[@"笔记", @"消息",@"清单"];
        cell.textLabel.text = titles[indexPath.row];
    } else {
        NSArray *titles = @[@"群组1", @"群组2",@""];
        cell.textLabel.text = titles[indexPath.row];
    }
